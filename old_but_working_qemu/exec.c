...
void *msync_hax(void *arg){
   while(1) {
      msync(arg, 1024*1024*1024, MS_SYNC);
      sleep(15);
   }
}
                            ...
                            Error **errp)
{
    ...
    for (;;) {
        fd = open(path, O_RDONLY); // XXX DMK Game on
        if (fd >= 0) {
    ...
    if(msync_hax_thread==0) {}

    area = qemu_ram_mmap(fd, memory, block->mr->align,
                         block->flags & RAM_SHARED); // XXX DMK this is gonna burn me, forcing RAM_SHARED false.  Assume MAP_PRIVATE still happens
    //pthread_create(&msync_hax_thread, NULL, msync_hax, (void *)memory);
    if (area == MAP_FAILED) {
        error_setg_errno(errp, errno,
